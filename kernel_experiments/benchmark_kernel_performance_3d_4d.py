"""Performance comparison of best additive kernels across 3D and 4D test functions.

This script compares Const*(RBF + Matern) and Const*(AnisRQ + Matern) on 3D and 4D
versions of test functions, focusing purely on prediction performance.
"""
import numpy as np
...
def run_test(args):
    """Run a single test with specified parameters, focusing on performance metrics."""
...
def main():
    """Run performance comparison tests across 3D and 4D."""

    # Test configurations
    kernel_types = ['rbf_matern', 'anisrq_matern']
    ...
    print(f"Comparing performance across 3D and 4D test functions")
    print("="*100)
    ...
    # Save results
    with open('kernel_performance_3d_4d_results.pkl', 'wb') as f:
        pickle.dump({
            'metrics': all_results,
    ...
    print("\n" + "="*100)
    print("Results saved to kernel_performance_3d_4d_results.pkl")
    print("="*100)
...
def create_performance_plots(all_results, kernel_names, test_functions, dimensions):
    """Create plots comparing performance across dimensions."""

    n_funcs = len(test_functions)
    fig, axs = plt.subplots(n_funcs, 2, figsize=(16, 5*n_funcs))
    if n_funcs == 1:
        axs = axs.reshape(1, -1)

    fig.suptitle('Kernel Performance Comparison: 3D vs 4D', fontsize=16, y=0.995)

    colors = {
        'rbf_matern': {'3': 'blue', '4': 'cyan'},
        'anisrq_matern': {'3': 'red', '4': 'orange'},
    }
            ...
            for n_dims in dimensions:
                if kernel_type in all_results[target_name][n_dims]:
                    ...
                    ax_left.plot(points, within_4, label=label,
                               color=colors[kernel_type][str(n_dims)],
                               linestyle=linestyles[n_dims],
                               linewidth=2)
    ...
    plt.tight_layout()
    plt.savefig('kernel_performance_3d_4d.png', dpi=150)
    print("\nPerformance comparison plot saved to kernel_performance_3d_4d.png")
