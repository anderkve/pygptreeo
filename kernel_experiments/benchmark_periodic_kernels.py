"""Benchmark comparing additive kernels including periodic components.

This script tests whether adding periodic kernels helps with oscillatory functions
...
def create_gpr_class(kernel_type, n_dims):
    ...
    class CustomGPR(GaussianProcessRegressor):
        def __init__(self, kernel=None, *, alpha=1e-6, optimizer='fmin_l_bfgs_b',
            ...
            if kernel_type == 'rbf_matern':
                # Baseline: Const*(RBF + Matern)
                ...
                # Const*(RBF + RationalQuadratic): two smooth kernels
                self.kernel = ConstantKernel(
                    constant_value=1.0,
                    constant_value_bounds=(1e-3, 1e8)
                ) * (RBF(
                ...
                # Const*(Matern + RationalQuadratic): alternative smooth combination
                self.kernel = ConstantKernel(
                    constant_value=1.0,
                    constant_value_bounds=(1e-3, 1e8)
                ) * (Matern(
            ...
            elif kernel_type == 'rbf_rbf':
                # Const*(RBF + RBF): two RBF kernels for multi-scale
                self.kernel = ConstantKernel(
                    constant_value=1.0,
                    constant_value_bounds=(1e-3, 1e8)
                ) * (RBF(
            ...
            self.alpha = 1e-6  # Normal regularization for all kernels
    ...
    for x, y in zip(X_input, y_input):
        ...
        if point_i % batch_size == 0 and point_i > 0:
            ...
            print(f"  {target_name}/{kernel_type}/{n_dims}D: {point_i}/{n_pts} pts, "
                  f"NRMSE={nrmse:.4f}, within_4%={within_4:.3f}")

    return (target_name, kernel_type, n_dims, all_metrics)


def main():
    """Run periodic kernel comparison tests."""

    # Test configurations - various additive kernel combinations
    kernel_types = [
        'rbf_matern',           # Baseline (best from previous tests)
        'rbf_rq',               # RBF + RationalQuadratic
        'matern_rq',            # Matern + RationalQuadratic
        'rbf_rbf',              # RBF + RBF (multi-scale smooth)
    ]

    kernel_names = {
        'rbf_matern': 'Const*(RBF + Matern)',
        'rbf_rq': 'Const*(RBF + RQ)',
        'matern_rq': 'Const*(Matern + RQ)',
        'rbf_rbf': 'Const*(RBF + RBF)',
    }

    # Test functions
    test_functions = ['eggholder', 'rastrigin', 'levy', 'schwefel']
    ...
    print(f"Testing periodic kernels on oscillatory functions")
    print("="*100)

    # Initialize results storage
    all_results = {}
    for target in test_functions:
        all_results[target] = {}
        for n_dims in dimensions:
            all_results[target][n_dims] = {}

    # Run tests in parallel with 4 processes
    with Pool(processes=4) as pool:
        for target_name, kernel_type, n_dims, metrics in pool.imap_unordered(run_test, test_args):
            # Store results
            all_results[target_name][n_dims][kernel_type] = metrics
            print(f"\nCompleted: {target_name}/{kernel_type}/{n_dims}D")

    # Save results
    with open('additive_kernel_results.pkl', 'wb') as f:
        pickle.dump({
            'metrics': all_results,
    ...
    for target_name in test_functions:
        ...
        for n_dims in dimensions:
            print(f"\n  {n_dims}D:")
            print(f"  {'Kernel':<40} {'NRMSE':<10} {'<1%':<8} {'<2%':<8} {'<4%':<8} {'<8%':<8}")
            print("  " + "-" * 94)

            for kernel_type in kernel_types:
                if kernel_type in all_results[target_name][n_dims]:
                    final_metrics = all_results[target_name][n_dims][kernel_type][-1]
                    print(f"  {kernel_names[kernel_type]:<40} "
    ...
    print("\n" + "="*100)
    print("Results saved to additive_kernel_results.pkl")
    print("="*100)

    # Create visualization
    create_performance_plots(all_results, kernel_names, test_functions, dimensions)


def create_performance_plots(all_results, kernel_names, test_functions, dimensions):
    """Create plots comparing periodic kernel performance."""

    n_funcs = len(test_functions)
    fig, axs = plt.subplots(n_funcs, 2, figsize=(18, 5*n_funcs))
    if n_funcs == 1:
        axs = axs.reshape(1, -1)

    fig.suptitle('Additive Kernel Comparison: Testing Different Kernel Combinations', fontsize=16, y=0.995)
    ...
    for i, target_name in enumerate(test_functions):
        ...
        # Left column: Fraction within 4% (3D only for clarity)
        for kernel_type in kernel_names.keys():
            ...
            if kernel_type in all_results[target_name][n_dims]:
                ...
                linewidth = 3 if kernel_type == 'rbf_matern' else 2
                ax_left.plot(points, within_4, label=label,
                           color=colors[kernel_type],
                           linewidth=linewidth,
                           alpha=0.9)

        ax_left.set_xlabel('Points Processed', fontsize=11)
        ax_left.set_ylabel('Fraction Within 4%', fontsize=11)
        ax_left.set_title(f'{target_name.title()} - 3D Accuracy', fontsize=12)
        ax_left.set_ylim([0, 1])
        ax_left.legend(fontsize=9, loc='best')
        ax_left.grid(True, alpha=0.3)

        # Right column: Fraction within 4% (4D only)
        for kernel_type in kernel_names.keys():
            n_dims = 4
            if kernel_type in all_results[target_name][n_dims]:
                metrics = all_results[target_name][n_dims][kernel_type]
                ...
                label = kernel_names[kernel_type]
                linewidth = 3 if kernel_type == 'rbf_matern' else 2
                ax_right.plot(points, within_4, label=label,
                            color=colors[kernel_type],
                            linewidth=linewidth,
                            alpha=0.9)

        ax_right.set_xlabel('Points Processed', fontsize=11)
        ax_right.set_ylabel('Fraction Within 4%', fontsize=11)
        ax_right.set_title(f'{target_name.title()} - 4D Accuracy', fontsize=12)
        ax_right.set_ylim([0, 1])
        ax_right.legend(fontsize=9, loc='best')
        ax_right.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig('additive_kernel_performance.png', dpi=150, bbox_inches='tight')
    print("\nPerformance comparison plot saved to additive_kernel_performance.png")
