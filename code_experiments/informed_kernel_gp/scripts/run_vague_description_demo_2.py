#!/usr/bin/env python
"""Demo 2: feature suggestion from a vague description — Himmelblau target.

Same workflow as run_vague_description_demo.py but with the 2D Himmelblau
function (scaled to [0,1]²) as the true log-likelihood.

The standard Himmelblau function is:
    f(x, y) = (x² + y - 11)² + (x + y² - 7)²

defined on [-5, 5]².  We rescale the inputs so that [0,1]² maps to [-5, 5]²
and negate the result so it behaves like a log-likelihood (four local maxima).
"""
...
# =====================================================================
# 2. Himmelblau function scaled to [0,1]²
# =====================================================================

def true_function(X):
    """Negated Himmelblau function rescaled so [0,1]² maps to [-5,5]².

    The standard Himmelblau function has four local minima:
      (3.0, 2.0), (-2.805, 3.131), (-3.779, -3.283), (3.584, -1.848)

    After negation these become four local maxima (like a log-likelihood).
    After rescaling, the [0,1]² optima are approximately:
      (0.80, 0.70), (0.22, 0.81), (0.12, 0.17), (0.86, 0.32)
    """
    # Rescale [0,1] -> [-5, 5]
    x = X[:, 0] * 10.0 - 5.0
    y = X[:, 1] * 10.0 - 5.0
    ...
    # Standard Himmelblau
    h = (x**2 + y - 11.0)**2 + (x + y**2 - 7.0)**2

    # Negate so maxima correspond to the optima (log-likelihood convention)
    return -h
...
# =====================================================================
# 4. Plotting helpers (same as demo 1)
# =====================================================================
...
def main():
    output_dir = os.path.join(
        os.path.dirname(__file__), "..", "outputs", "vague_description_2"
    )
    ...
    print("=" * 70)
    print(f"\nTrue function: negated Himmelblau on [0,1]²")
    print(f"  f(X) = -[(x² + y - 11)² + (x + y² - 7)²]")
    print(f"  with x = 10*X[:,0] - 5, y = 10*X[:,1] - 5")
    print(f"  Four local maxima (optima) in [0,1]²\n")
    ...
    plot_learning_curves(
        ...
        subtitle="Vague description → suggested features (Himmelblau target)"
    )
    ...
    print("=" * 70)
    print()
    print("Target: negated Himmelblau function (4 local maxima on [0,1]²)")
