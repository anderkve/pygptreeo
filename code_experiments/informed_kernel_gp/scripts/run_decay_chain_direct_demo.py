#!/usr/bin/env python
"""Demo: fitting the Bateman decay function directly (not a log-likelihood).

Complements run_decay_chain_demo.py by testing whether physics-informed
features help when the GP target is the raw intermediate-species
concentration N_B(λ₁, λ₂) from the Bateman equations, rather than a
log-likelihood surface built on top of it.

The true function is:

    f(u, v) = N_B(t*; λ₁(u), λ₂(v))

where t* is a fixed measurement time and (u,v) ∈ [0,1]² maps linearly
to decay rates (λ₁, λ₂) ∈ [λ_min, λ_max]².

This surface has the characteristic features of the Bateman solution:
exponential fall-off, a ridge near λ₁ ≈ λ₂, and a peak whose location
depends on t*.
"""
...
# =====================================================================
# 1. User description (mentions decay chain, but NOT log-likelihood)
# =====================================================================

USER_DESCRIPTION = (
    "The function represents the concentration of the intermediate "
    "species in a radioactive decay chain A → B → C, evaluated at a "
    "fixed measurement time. The two input parameters are the decay "
    "rates of the first and second stages. The function is smooth and "
    "has a single peak, with a ridge-like structure near the line where "
    "the two rates are equal. Input range is [0,1] in each coordinate."
)


# =====================================================================
# 2. True function: raw Bateman N_B
# =====================================================================

LAMBDA_MIN = 0.5
LAMBDA_MAX = 5.0
N0 = 100.0
T_STAR = 1.0  # fixed measurement time


def _bateman_NB_scalar(t, lam1, lam2):
    """N_B(t) for a single (lam1, lam2) pair."""
    dt = lam2 - lam1
    if abs(dt) > 1e-8:
        return N0 * lam1 / dt * (np.exp(-lam1 * t) - np.exp(-lam2 * t))
    else:
        return N0 * lam1 * t * np.exp(-lam1 * t)


def true_function(X):
    """Bateman intermediate-species concentration at t = T_STAR.

    Parameters
    ----------
    X : ndarray, shape (n, 2)
        Each row is (u, v) ∈ [0,1]², mapped to (λ₁, λ₂).
    ...
    for i in range(n):
        ...
        y[i] = _bateman_NB_scalar(T_STAR, lam1, lam2)
    return y
...
def main():
    output_dir = os.path.join(
        os.path.dirname(__file__), "..", "outputs", "decay_chain_direct"
    )
    ...
    print("=" * 70)
    print("STEP 3: Benchmark — fitting raw Bateman N_B directly")
    print("=" * 70)
    print(f"\nTrue function: N_B(t*; λ₁, λ₂)  [Bateman intermediate species]")
    print(f"  t* = {T_STAR}")
    print(f"  N₀ = {N0}")
    print(f"  (λ₁, λ₂) ∈ [{LAMBDA_MIN}, {LAMBDA_MAX}]² → rescaled to [0,1]²")
    ...
    plot_learning_curves(
        ...
        subtitle="Raw Bateman $N_B(\\lambda_1, \\lambda_2)$ — physics-informed features"
    )
    ...
    print("=" * 70)
    print()
    print("Target: raw Bateman N_B(t*; λ₁, λ₂) on [0,1]²")
    print("(Direct physical function, NOT a log-likelihood)")
    print()
    for n in [10, 20, 50]:
        if n in results:
