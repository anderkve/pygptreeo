#!/usr/bin/env python
"""Demo: emulating a numerical *double* integral with no closed-form expression.

This is a harder variant of the single-integral demo.  The true function
involves a double integral over a bounded domain, which makes it more
challenging for an agent to reason about the parametric dependence and
suggest good basis functions.

The true function is:

    f(a, b) = ∫₀^π ∫₀^π  exp(-(s-t)²) · sin(a·s + b·t)
                            · log(1 + s² + t²)  ds dt

where (a, b) are mapped from (u, v) ∈ [0,1]².

Key properties that make this harder than the single-integral case:
  • exp(-(s-t)²) concentrates the integrand near the diagonal s ≈ t,
    coupling the two integration variables non-trivially.
  • sin(a·s + b·t) creates oscillatory dependence on both parameters a
    and b.  As a or b increase, the integrand oscillates more rapidly
    in s or t, leading to partial cancellation and a non-monotone
    surface with sign changes.
  • The agent must reason about the Fourier-like structure of
    ∫ sin(αx) · g(x) dx and how the Gaussian coupling modifies it
    for a double integral.

An agent inspecting this code can reason:
  • Near the diagonal s ≈ t, write t = s + δ where δ is small:
      sin(a·s + b·t) ≈ sin((a+b)·s + b·δ) ≈ sin((a+b)·s)·cos(b·δ)
    So the integral's dominant behavior depends on (a+b) through
    a Fourier-sine-like integral:
      ∫₀^π sin((a+b)·s) · envelope(s) ds
  • This Fourier structure gives oscillatory features like:
      (1 - cos((a+b)π)) / (a+b)   and   sin((a+b)π) / (a+b)
  • Off-diagonal corrections (δ ≠ 0) introduce dependence on (a-b)
    through cos(b·δ) → sin((a-b)·...) type terms.
  • For large (a+b), the rapid oscillation of sin causes cancellation,
    making f small.  For (a+b) near even integers, cos((a+b)π) ≈ 1,
    giving near-zero values (destructive interference).
"""
...
# =====================================================================
# 1. True function: parametric double integral (no closed form)
# =====================================================================

A_MIN, A_MAX = 0.5, 8.0    # frequency parameter for s
B_MIN, B_MAX = 0.5, 8.0    # frequency parameter for t
F_OFFSET = 2.0             # shift to keep values positive for rel. error


def _integrand(t, s, a, b):
    """Integrand: exp(-(s-t)²) · sin(a·s + b·t) · log(1 + s² + t²).

    Note: scipy.integrate.dblquad calls the inner integrand as f(t, s),
    i.e. the inner variable first.
    """
    return (np.exp(-(s - t) ** 2) * np.sin(a * s + b * t)
            * np.log(1.0 + s ** 2 + t ** 2))


def _f_scalar(a, b):
    """Evaluate the double integral for a single (a, b) pair."""
...
def true_function(X):
    """Double integral on [0,1]² → (a, b) parameter space.
...
# double-integral code and reasoning about the parametric dependence.
...
def _get_agent_suggested_features():
    ...
    The agent reads the integrand:
        exp(-(s-t)²) · sin(a·s + b·t) · log(1 + s² + t²)
    and reasons about how f(a, b) depends on the parameters through
    the double integration over [0,π]².
    """
    features = []
    labels = []

    def _a(X):
        return A_MIN + X[:, 0] * (A_MAX - A_MIN)

    def _b(X):
        return B_MIN + X[:, 1] * (B_MAX - B_MIN)

    # --- Fourier-sine features from the diagonal approximation ---
    # Reasoning: near the diagonal s ≈ t, the integrand behaves like
    # sin((a+b)·s) × envelope(s).  Integrating ∫₀^π sin(ω·s) ds
    # = (1 - cos(ω·π)) / ω.  This is the dominant parametric
    # dependence through ω = a+b.
    features.append(lambda X: (1.0 - np.cos((_a(X) + _b(X)) * np.pi)) / (_a(X) + _b(X)))
    labels.append("(1-cos((a+b)π))/(a+b)")

    # --- Companion cosine feature ---
    # Reasoning: the integral ∫₀^π s·sin(ω·s) ds involves
    # sin(ω·π)/ω² terms.  This captures the first-order correction
    # from the envelope shape.
    features.append(lambda X: np.sin((_a(X) + _b(X)) * np.pi) / (_a(X) + _b(X)))
    labels.append("sin((a+b)π)/(a+b)")

    # --- Higher-order Fourier correction ---
    # Reasoning: ∫₀^π s·cos(ω·s) ds type terms appear from
    # integration by parts against the log envelope.
    features.append(lambda X: np.sin((_a(X) + _b(X)) * np.pi) / (_a(X) + _b(X)) ** 2)
    labels.append("sin((a+b)π)/(a+b)²")

    # --- Off-diagonal (a-b) features ---
    # Reasoning: the Gaussian exp(-(s-t)²) has width ~1.  Away from
    # the diagonal, sin(a·s+b·t) = sin((a+b)·s/2 + (a-b)·(s-t)/2 + ...)
    # introduces dependence on (a-b).
    features.append(lambda X: (
        (1.0 - np.cos((_a(X) - _b(X)) * np.pi))
        / (np.abs(_a(X) - _b(X)) + 0.1)
    ))
    labels.append("(1-cos((a-b)π))/(|a-b|+ε)")

    features.append(lambda X: np.sin((_a(X) - _b(X)) * np.pi) / (_a(X) - _b(X) + 0.1))
    labels.append("sin((a-b)π)/(a-b+ε)")

    # --- Scale features ---
    # Reasoning: the overall magnitude of the integral scales with
    # 1/(a+b) because faster oscillation → more cancellation.
    features.append(lambda X: 1.0 / (_a(X) + _b(X)))
    labels.append("1/(a+b)")

    features.append(lambda X: 1.0 / (_a(X) + _b(X)) ** 2)
    labels.append("1/(a+b)²")

    # --- Individual parameter features ---
    # Reasoning: even though the diagonal approximation depends on
    # (a+b), the actual double integral is NOT perfectly described
    # by (a+b) alone.  Individual a, b features capture this.
    features.append(lambda X: (1.0 - np.cos(_a(X) * np.pi)) / _a(X))
    labels.append("(1-cos(aπ))/a")

    features.append(lambda X: (1.0 - np.cos(_b(X) * np.pi)) / _b(X))
    labels.append("(1-cos(bπ))/b")
...
def plot_predictive_surfaces(X_test, y_test, n_test_per_dim, output_dir,
    ...
    for ax in axes.flat:
        if ax.get_visible():
            ax.set_aspect("equal")
            ax.set_xlabel(r"$a$ (frequency param for $s$, rescaled)")
            ax.set_ylabel(r"$b$ (frequency param for $t$, rescaled)")
...
def main():
    output_dir = os.path.join(
        os.path.dirname(__file__), "..", "outputs", "numerical_double_integral"
    )
    os.makedirs(output_dir, exist_ok=True)

    # --- Step 1: Display the true function ---
    print("=" * 70)
    print("STEP 1: True function (double numerical integral, no closed form)")
    print("=" * 70)
    print()
    print("  f(a, b) = ∫₀^π ∫₀^π exp(-(s-t)²) · sin(a·s + b·t)")
    print("                          · log(1 + s² + t²)  ds dt")
    print()
    print(f"  a ∈ [{A_MIN}, {A_MAX}]  (frequency parameter for s)")
    print(f"  b ∈ [{B_MIN}, {B_MAX}]  (frequency parameter for t)")
    print(f"  Mapped from (u, v) ∈ [0,1]²")
    print(f"  Offset = {F_OFFSET} added to keep values positive")
    print()
    ...
    print("=" * 70)
    print()
    print("  An agent inspecting the double-integral code would reason:")
    print("    • exp(-(s-t)²) concentrates integrand near diagonal s ≈ t")
    print("    • On the diagonal: sin(a·s + b·t) ≈ sin((a+b)·s)")
    print("    • Fourier structure: ∫ sin(ω·s)·g(s) ds → features in ω = a+b")
    print("    • Off-diagonal corrections introduce (a-b) dependence")
    print("    • Oscillatory cancellation for large (a+b)")
    print()
    ...
    print("=" * 70)
    print("STEP 3: Benchmark — emulating the double numerical integral")
    print("=" * 70)
    ...
    plot_learning_curves(
        ...
        subtitle="Double numerical integral — agent-suggested features from code inspection",
    )
    ...
    print("=" * 70)
    print()
    print("Target: f(a,b) = ∫₀^π ∫₀^π exp(-(s-t)²)·sin(as+bt)·log(1+s²+t²) ds dt")
    print("(Double integral with no closed-form solution)")
    print()
    for n in N_TRAIN_VALUES:
        if n in results:
