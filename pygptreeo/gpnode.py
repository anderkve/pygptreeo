...
class GPNode(Node):
    ...
    def generate_children(self, GPR: Type[GaussianProcessRegressor], n_features: int):
        ...
        self.left = GPNode(0, my_GPR=deepcopy(self.my_GPRs[0]), name=self.name + "0", **node_config_kwargs)
        self.right = GPNode(0, my_GPR=deepcopy(self.my_GPRs[0]), name=self.name + "1", **node_config_kwargs)
        ...
        if self.use_hyperparameter_inheritance:
            # For each output, copy the trained kernel to children
            for i in range(self.n_outputs):
                if hasattr(self.my_GPRs[i], 'kernel_'):
                    self.left.my_GPRs[i].kernel = deepcopy(self.my_GPRs[i].kernel_)
                    self.right.my_GPRs[i].kernel = deepcopy(self.my_GPRs[i].kernel_)
            if self.n_outputs == 1:
    ...
    def should_merge_point(self, x: np.ndarray, y: Union[float, np.ndarray],
        ...
        # GP not trained yet (check first GP in list)
        if not hasattr(self.my_GPRs[0], 'kernel_'):
            return False
