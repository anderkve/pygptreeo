...
class GPRegressorInterface(ABC):
    ...
    def supports_incremental_update(self) -> bool:
        """
        Whether this backend supports exact rank-1 incremental updates.

        Backends that return True must implement :meth:`add_observation`. The
        default is False, which means the tree always uses full :meth:`fit`
        calls for this backend.

        Returns
        -------
        bool
            True if incremental updates are supported, False otherwise.
        """
        return False

    def add_observation(self, x: np.ndarray, y: float, alpha: float) -> None:
        """
        Incorporate a single new observation with fixed hyperparameters.

        This is used by the tree to update a leaf's GP between periodic full
        re-fits, so that the posterior reflects every new point at O(n^2) cost
        rather than O(n^3). Only called when :meth:`supports_incremental_update`
        returns True.

        Parameters
        ----------
        x : np.ndarray
            New input point of shape (1, n_features) or (n_features,).
        y : float
            New target value.
        alpha : float
        ...
        raise NotImplementedError(
            "This GP backend does not support incremental updates."
        )
