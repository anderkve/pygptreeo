...
class OrderAdditiveKernel(StationaryKernelMixin, Kernel):
    ...
    This is the Duvenaud et al. (2011) additive-GP construction (the same
    backbone as OAK, Lu et al. 2022) and an alternative to :class:`AdditiveKernel`.
    Both model a function as a sum of low-order interaction terms, but they
    assemble that sum very differently:

    - :class:`AdditiveKernel` enumerates every interaction term explicitly, so a
      depth-``D`` kernel over ``d`` inputs builds ``sum_{n<=D} C(d,n)`` product
      matrices on each evaluation -- ``O(d^D)`` work.
    - ``OrderAdditiveKernel`` writes the order-``n`` contribution as the ``n``-th
      elementary symmetric polynomial ``e_n`` of the ``d`` one-dimensional
      kernels and obtains all of ``e_1..e_D`` from the power sums by the
      Newton-Girard recursion in ``O(d*D)`` elementwise matrix operations --
      independent of ``C(d,n)``. This is dramatically cheaper for larger ``d``
      and especially for ``max_order >= 3``.

    The kernel is

        k(x, x') = sum_{n=1..D}  v_n * e_n( z_1, ..., z_d ),

    where ``z_i = k_i(x_i, x'_i)`` is the 1-D base kernel for dimension ``i`` (its
    own length scale ``l_i``) and ``v_n`` is a single variance shared by all
    order-``n`` terms. Hyperparameters are therefore ``d`` length scales + ``D``
    order variances (``d + D`` in total), independent of the number of terms --
    so this adds essentially no parameters over the shared-amplitude
    :class:`AdditiveKernel` and does not hurt sample efficiency in small leaves.
    Gradients (w.r.t. log length scales and log order variances) are analytic.
    ...
    Examples
    --------
    >>> from pygptreeo.kernels import OrderAdditiveKernel
    >>> from sklearn.gaussian_process import GaussianProcessRegressor
    >>> kernel = OrderAdditiveKernel(input_dim=6, max_order=2)
    >>> gpr = GaussianProcessRegressor(kernel=kernel)
    """
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        # d k / dlog l_i = ( sum_n v_n e_{n-1}^{\i} ) * dz_i/dlog l_i, where
        # e_m^{\i} is the m-th elementary symmetric polynomial of all dims but i.
        for i in range(self.input_dim):
...
def make_order_additive_kernel(n_features, max_order=2, base_kernel='matern',
                               ...
                               rescue_length_scale_bounds=(1e-5, 1e5)):
    """Build a per-order additive kernel (:class:`OrderAdditiveKernel`) plus the
    same full-D Matern "rescue" term as :func:`make_additive_kernel`.

    This is the Newton-Girard counterpart of :func:`make_additive_kernel`: same
    sample-efficiency benefit and same safety net (the rescue term lets the model
    fall back to the plain Matern on non-additive targets), but with much cheaper
    kernel assembly that scales to higher ``max_order`` / dimension. The per-order
    variances already supply the additive amplitudes, so -- unlike
    :func:`make_additive_kernel` -- no extra ``ConstantKernel`` is placed in front
    of the additive block.

    Parameters mirror :func:`make_additive_kernel`; see :class:`OrderAdditiveKernel`
    for the additive-block hyperparameters.

    Returns
    -------
    sklearn.gaussian_process.kernels.Kernel
        A kernel ready for a ``GaussianProcessRegressor`` (e.g. via
        ``Default_GPR(kernel=...)`` or a ``SklearnGPAdapter``).

    Examples
    --------
    >>> from pygptreeo import GPTree, Default_GPR, make_order_additive_kernel
    >>> kernel = make_order_additive_kernel(n_features=6, max_order=2)
    >>> gpt = GPTree(GPR=Default_GPR(kernel=kernel, alpha=1e-6))
    """
