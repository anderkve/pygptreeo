...
*pyGPTreeO learning a 2D target function (Himmelblau) online, from a stream of input points. The points do not cover the whole input space — they arrive as a moving, concentrated cluster that sweeps a diagonal band. **Left:** the unknown target function with the incoming data drawn on top (grey = seen earlier, red = just arrived). **Middle:** pyGPTreeO's current prediction, shown only as strongly as the model is confident there (opacity = confidence); the thin white rectangles are the leaves of the GP tree — local GP models that the tree grows where data is dense. **Right:** the predictive uncertainty. The key point: the target is learned, and the uncertainty is low, only in the regions where input points have actually been received — the prediction "fills in" along the data's path while the never-visited corners stay unknown. Generated by [`examples/make_readme_gif.py`](examples/make_readme_gif.py).*
