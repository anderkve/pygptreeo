...
*pyGPTreeO learning a 2D target (Himmelblau) online from a stream of input points that arrive as a moving cluster sweeping across the domain. **Left to right:** the unknown target with the incoming data (white = seen earlier, red = just arrived); pyGPTreeO's prediction, overlaid with the white GP-tree partition and a cyan contour marking where the relative uncertainty falls below 1%; then the relative predictive uncertainty and the relative error (log scale, 0.1%–100%). The target is learned — low uncertainty *and* low error — only where input points have actually been received. Generated by [`examples/make_readme_gif.py`](examples/make_readme_gif.py).*
