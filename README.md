...
![pyGPTreeO learning a 2D target function online from a stream of input points](examples/example_plots/animation/pygptreeo_local_learning_nbar100.gif)

*pyGPTreeO learning a 2D target function (Himmelblau) online, from a stream of input points. The points do not cover the whole input space — they mimic an optimiser/adaptive sampler: a cluster that starts broad and gradually shrinks as its centre sweeps across the domain, finally converging onto a minimum of the target function (the upper-right one) which it then samples in high detail. **Left:** the unknown target function with the incoming data drawn on top (small white dots = seen earlier, red = just arrived). **Middle:** pyGPTreeO's current prediction, with a cyan contour enclosing the region where the relative predictive uncertainty is below 1% (the same contour is overlaid on the uncertainty and error panels); the thin white rectangles are the leaves of the GP tree — local GP models that the tree grows where data is dense. **Right of that:** the relative predictive uncertainty, and **far right:** the relative prediction error against the true function — both on a logarithmic scale from 0.1% to 100%. The key point: the target is learned — low relative uncertainty *and* low relative error — only in the regions where input points have actually been received. The prediction "fills in" where the data goes while regions that are never sampled stay unknown. Generated by [`examples/make_readme_gif.py`](examples/make_readme_gif.py).*
