# Test file for standard scaling feature
import numpy as np
...
def test_gp_standard_scaling():
    # Test function for standard scaling in GPTree
    ...
    X_test = np.array([[1.05, 2.05], [5.05, 6.05], [10.15, 11.15]])
    ...
    print(tree_scaled.root)

    found_leaf_with_data_and_scaler = False
    for leaf in tree_scaled.root.leaves:
        print(f"Checking scaled leaf: {leaf.name}, num_training_points: {leaf.num_training_points}, my_X_data shape: {leaf.my_X_data.shape if leaf.my_X_data is not None else 'None'}") # Added more debug info
        ...
        if leaf.my_GPR is not None and leaf.num_training_points > 0:
            # Check if my_X_data is also present, as fit_my_GPR needs it
            ...
            if leaf.my_X_data is not None and leaf.my_X_data.shape[0] > 0:
                ...
                # Added return_std=True to GPR predict call
    ...
    # Assert that predictions are different (scaling should have an effect)
