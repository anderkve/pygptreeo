...
def plot_marginals(assisted, refs, problems, out_path,
    ...
    if not refs:
        return
    n_dims_show = max(1, max(refs[k]["samples"].shape[1] for k in refs))
    n_dims_show = min(n_dims_show, n_dims_max)
...
def main():
    ...
    ap.add_argument("--tau-pick", type=float, default=1e-2)
    ap.add_argument("--method-pick", default="pygptreeo_A")
    args = ap.parse_args()
    ...
    plot_marginals(
        ...
        os.path.join(args.out_dir, "assisted_marginals.png"),
        tau_pick=args.tau_pick,
    )
    plot_corner(
        assisted, refs, args.problems,
        os.path.join(args.out_dir, "assisted_corner.png"),
        tau_pick=args.tau_pick, method_pick=args.method_pick,
    )
